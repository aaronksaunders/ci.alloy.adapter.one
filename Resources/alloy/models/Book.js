        ...
        _.extend(this, {
                ...
                Ti.API.info("findAll"), params && _.extend(_options, params), that.Cloud.Places.query(_options, function(e) {
                    if (e.success) {
                        if (e.places.length != 0) {
                            var d = [];
                            for (var i in e.places) d.push(new Place(e.places[i]));
                            _callback(d);
                        }
                    } else _callback(null);
                });
            ...
            update: function(model, callback) {
                Ti.API.info("update:model " + JSON.stringify(model)), that.Cloud.Places.update({
                    ...
                    e.success ? (Ti.API.info("success"), callback(new Place(e.places[0]))) : (Ti.API.error("ERROR"), callback(null));
                });
            },
            ...
            find: function(model, _callback) {
                ...
                Ti.API.info("find"), that.Cloud.Places.show({
                    ...
                    e.success ? e.places.length != 0 && _callback(new Place(e.places[0])) : _callback(null);
                });
            },
